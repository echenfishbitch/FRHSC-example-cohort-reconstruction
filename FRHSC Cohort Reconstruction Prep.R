###############################################
######### Feather River Hatchery Spring CWT Reconstruction ######
#################CWT Data Prep#################
###############################################
library(dplyr)
library(tidyr)
...
#turning NAs into 0 for addition
CWT_Recoveries$estimated_number[is.na(CWT_Recoveries$estimated_number)] <- 0
...
CWT_Hatchery<-CWT_Recoveries %>%
  ...
  group_by(brood_year) %>%
  summarize(Age2Hat = sum(`2`, na.rm = TRUE), Age3Hat = sum(`3`, na.rm = TRUE), Age4Hat = sum(`4`, na.rm = TRUE), Age5Hat = sum(`5`, na.rm = TRUE))
...
write.csv(CWT_SG, "Escape to Spawning Grounds.csv", row.names = FALSE)
...
for(j in 1:niterations){
  ...
  for(i in 1:length(CWT_SG$Individuals)){
...
CWT_River<- CWT_Recoveries %>%
  select(reporting_agency,run_year,period, fishery, sex, tag_code, estimated_number, recovery_location_code, brood_year, estimated_number, Phi) %>%
  filter(fishery == 46) %>% #fromriver fisheries
  mutate(Age = run_year - brood_year) %>%
  group_by(run_year, brood_year, Age) %>%
  summarise(Individuals= sum(estimated_number/Phi)) %>%
  pivot_wider(names_from = Age, values_from = Individuals, names_sort=TRUE) %>%
  group_by(brood_year) %>%
  summarize(InRiver2 = sum(`2`, na.rm = TRUE), InRiver3 = sum(`3`, na.rm = TRUE)
            , InRiver4 = sum(`4`, na.rm = TRUE), InRiver5 = sum(`5`, na.rm = TRUE))
